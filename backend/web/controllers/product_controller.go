...
func (p *ProductController) GetManager() mvc.View { //商品管理界面，负责修改
	idString := p.Ctx.URLParam("id")              //获取id
	id, err := strconv.ParseInt(idString, 10, 16) //由10进制转为16进制
	if err != nil {
		p.Ctx.Application().Logger().Debug(err) //打印error
	}
...
func (p *ProductController) GetDelete() { //删除
