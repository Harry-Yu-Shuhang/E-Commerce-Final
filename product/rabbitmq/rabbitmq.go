...
const MQURL = "amqp://imoocuser:imoocuser@172.28.86.108:5672/imooc" //改成rabbitmq服务器ip
...
func (r *RabbitMQ) ConsumeSimple(orderService services.IOrderService, productService services.IProductService) {
	...
	//消费者流控,可以防止爆库
	r.channel.Qos(
		1,     //当前消费者一次能接受的最大消息数量，每次从rabbitmq里面只接受1个消息
		0,     //服务器传递的最大容量（以八位字节为单位）
		false, //如果设置为true 对channel可用，这里只对当前消费队列进行设置
	)
